...
var sub={
			...
			chrome.tabs.query({highlighted:true,currentWindow:true},function(tabs){
				var theFunction=function(){
					...
					chrome.topSites.get(function(sites){
						...
						chrome.scripting.executeScript({code:'chrome.runtime.sendMessage({type:"apps_test",apptype:"'+_appname+'",value:sue.apps.enable,ctm:true,appjs:appType["'+_appname+'"]},function(response){console.log(response)})',runAt:"document_start"});
					});
				}
	...
	insertTest:function(appname){
		...
		chrome.scripting.executeScript({code:'chrome.runtime.sendMessage({type:"apps_test",apptype:"'+appname+'",value:sue.apps.enable,appjs:appType["'+appname+'"]},function(response){console.log(response)})',runAt:"document_start"});	
	},
	...
	action:{
		...
		//group nav
		back:function(){//chk
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, function: () => { window.history.go(-1); }})
		},
		...
		scroll:function(){
			...
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, files: ["js/inject/scroll.js"]})
		},
		reload:function(){//chk
			...
			if(!chrome.tabs.reload){
				for(var i=0;ids&&i<ids.length;i++){
					chrome.scripting.executeScript({target: {tabId: ids[i], allFrames: true}, function: () => { location.reload(); }});
				}
			}
			for(var i=0;ids&&i<ids.length;i++){
		...
		stop:function(){//chk
			var ids=sub.getId(sub.getConfValue("selects","n_tab"));
			for(var i=0;ids&&i<ids.length;i++){
				chrome.scripting.executeScript({target: {tabId: ids[i], allFrames: true}, function: () => { window.stop(); }})
			}
		},
		next:function(){
			if(sub.message.npok){
				...
				chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, files: ["js/namespace.js"]})	
				chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, files: ["js/inject/np.js"]})				
			}
		},
		previous:function(){
			sub.action.next();
		...
		copyimg:function(){
			if(!sub.message.selEle.img){return;}
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, files: ["js/inject/copyimg.js"]})
		},
		imgsearch:function(){
			if(!sub.message.selEle.img){return;}
		...
		closeapps:function(){
			...
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, function: () => {
		...
		script:function(){
			...
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id}, function: (scriptContent) => { eval(scriptContent); }, args: [config.general.script.script[_script].content]})
		},
		source:function(){
			var theTarget=sub.getConfValue("selects","n_optype"),
		...
		zoom:function(){
			if(!chrome.tabs.setZoom){
				...
				chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, files: ["js/inject/zoom.js"]})
			}else{
				var factorDefault=1;
		...
		print:function(){
			chrome.scripting.executeScript({target: {tabId: sub.curTab.id, allFrames: true}, function: () => { window.print(); }})
		},
	...
	apps:{
		...
		jslist:{
			jsRun:function(message){
				chrome.scripting.executeScript({target: { tabId: sub.curTab.id }, function: (scriptContent) => { eval(scriptContent); }, args: [config.general.script.script[message.value].content]})
			}
		},
		appslist:{
		...
		extmgm: {
            action: function(message, sender, sendResponse) {
                switch (message.value.actionType) {
                    ...
                    case "disable":
                        ...
                        chrome.management.setEnabled(message.value.id, message.value.actionType == "enable" ? true : false, () => {});
                        break;
                    case "disableall":
                        // Using Promise-based management API for better async handling
                        chrome.management.getAll().then(exts => {
                            for (let ext of exts) {
                                if (ext.enabled && ext.id !== sub.extID && (!config.apps.extmgm.always || !config.apps.extmgm.always.contains(ext.id))) {
            ...
            getAllExt: function(message, sender, sendResponse) {
                ...
                if (message.extsEnable) {
                    ...
                    chrome.management.getAll().then(exts => {
                        for (let ext of exts) {
                            if (ext.id == sub.extID || (config.apps.extmgm.always && config.apps.extmgm.always.contains(ext.id))) {
                                continue;
                            }
            ...
            itemUninstall: function(message, sender, sendResponse) {
                // Using Promise-based management API for better async handling
                chrome.management.uninstall(message.extId, { showConfirmDialog: config.apps.extmgm.n_uninstallconfirm })
                    .then(() => {
                ...
                fetch(message.value)
                    ...
                    .then(blob => {
                        ...
                        reader.onload = function(e) {
                            ...
                            const domInfos = htmlData.querySelector(".container .detail_intro tbody").querySelectorAll("tr");
                            ...
                            const domDls = htmlData.querySelector(".container #ziy .resourcesmain tbody").querySelectorAll("tr");
        ...
        shorturl: {
            getURL: async function(message, sender, sendResponse) {
                console.log(message);
                try {
                    const url = (config.apps.shorturl.n_suyourls ? "https://url.zimoapps.com/yourls-api.php" : config.apps.shorturl.n_yourls + "/yourls-api.php") +
                        "?action=shorturl" +
                        "&format=json" +
...
if (chrome.browserSettings && chrome.browserSettings.contextMenuShowEvent) {
    ...
    config.general.linux.cancelmenu = false;
    sub.saveConf();
} else if (browserType == "fx") { // Simplified logic, localStorage is not available in MV3
